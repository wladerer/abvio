...
class CheckIncar:
    ...
    def __init__(self, incar_dict: dict):
        ...
        self.reference_file = os.path.join(
            Path(__file__).parent, "reference", "incar.yaml"
        )
        self.messages: list[str] = []

    def check_magnitudes(self):
        incar_tags = lower_keys(
            yaml.load(open(self.reference_file), Loader=yaml.FullLoader)
        )
    ...
    def check_nbands(self, structure: Structure):
        ...
        if "nbands" in self.incar_dict:
            ...
            if user_nbands < estimated_nbands:
                self.messages.append(
                    f"NBANDS is too low: {user_nbands} < {estimated_nbands}"
                )
...
class CheckStructure:
    ...
    def check_positions(self, min_distance: float = 0.5) -> list[str]:
        ...
        for k in too_close[0]:
            ...
            self.messages.append(
                f"{symbols[i]}{i} is too close to {symbols[j]}{j} with a distance of {distance_matrix[i, j]:.3f} Å"
            )
